...
public class GsonConverterWithDecodeFactory extends Converter.Factory {

    /**
     * Create an instance using a default {@link Gson} instance for conversion. Encoding to JSON and
     * decoding from JSON (when no charset is specified by a header) will use UTF-8.
     */
    public static GsonConverterWithDecodeFactory create() {
    ...
    public static GsonConverterWithDecodeFactory create(Gson gson) {
        if (gson == null) {
            throw new NullPointerException("gson == null");
        }
        return new GsonConverterWithDecodeFactory(gson);
    }

    private final Gson gson;

    private GsonConverterWithDecodeFactory(Gson gson) {
        this.gson = gson;
    }
    ...
    private class GsonResponseBodyConverter<T> implements Converter<ResponseBody, T> {
        ...
        public T convert(ResponseBody value) throws IOException {
            ...
            String decrypt = XXTEA.decrypt(response);
//            LogoutManager.isLogout(decrypt);
            LogUtil.Companion.d("接口返回数据解密后-》" + decrypt);
            try {
